...
public interface FoodRepository extends CrudRepository<FoodEntity, Long> {
    ...
    @Query("SELECT e FROM FoodEntity e WHERE e.name = :name AND e.foodType = :foodType AND e.petType = :petType AND e.taste = :taste")
    FoodEntity getFoodWeAlreadyHave(@Param("name") String name, @Param("foodType") FoodType foodType, @Param("petType") PetType petType, @Param("taste") String taste);
